...
class WAM:
    def __init__(self):
        ...
        rospy.Subscriber('keys',String,self.move_wam)
        self.rate = rospy.Rate(10)

        print('ros node initialized')
        

    def move_wam(self,msg):
        ...
        #Update position
        joint_to_move = key_mapping[msg.data[0]][0] 
        sign = key_mapping[msg.data[0]][1]

        if   (sign == '+'):
            current_joints_dict[joint_to_move] += 0.005
        elif (sign == '-'):
            current_joints_dict[joint_to_move] -= 0.005
        else:
            return
